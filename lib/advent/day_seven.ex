defmodule Advent.DaySeven do
  ...
  iex>Advent.DaySeven.part_one([16,1,2,0,4,2,7,1,2,14])
  37
  """
  def part_one(input) do
    Enum.min(input)..Enum.max(input)
    |> Enum.map(&align(&1, input))
    |> Enum.min()
  end

  def align(num, numbers) do
    numbers
    |> Enum.map(&abs(num - &1))
    |> Enum.sum()
  end
  ...
  ## Examples 
  #
  iex>Advent.DaySeven.part_two([16,1,2,0,4,2,7,1,2,14])
  168
  """
  def part_two(input) do
    Enum.min(input)..Enum.max(input)
    |> Enum.map(fn lvl -> _align(lvl, input) end)
    |> Enum.min()
  end

  def _align(num, numbers) do
    numbers
    |> Enum.map(fn n -> Enum.sum(Range.new(0, abs(num - n))) end)
    |> Enum.sum()
  end
end
